...
public class zFoxVirtualPad : MonoBehaviour {
	...
	//内部パラメーター
	Camera uicam;
	SpriteRenderer sprSliderPad;
	SpriteRenderer sprSliderPadBack;
	...
	void Awake() {
		...
		sprSliderPad = GameObject.Find("SlidePad").GetComponent<SpriteRenderer>() as SpriteRenderer;
		sprSliderPadBack = GameObject.Find("SlidePadBack").GetComponent<SpriteRenderer>() as SpriteRenderer;
	...
	void RunAutoLayout() {
		if(autoLayout) {
			...
			sprSliderPadBack.transform.localPosition = posPad;
	...
	void Update () {
		...
		sprSliderPad.transform.localPosition = pos;
	}
	...
	bool CheckSlidePadDown(Vector2 posTouch) {
		if(posTouch.x / Screen.width < 0.5f) {
			...
			vec3.z = sprSliderPad.transform.position.z;
			sprSliderPadBack.transform.position = vec3;
			return true;
		}
		return false;
	}
	bool CheckSlidePadMove(Vector2 posTouch) {
		if(movPadEnable) {
			movEnable = true;
			mov = posTouch - movSt;
			sprSliderPad.color = new Color(1.0f, 0.0f, 0.0f, 1.0f);
            return true;
		}
		return false;
	}
	void CheckSlidePadUp() {
		sprSliderPad.color = new Color(1.0f, 1.0f, 1.0f, 0.2f);
	}
}
